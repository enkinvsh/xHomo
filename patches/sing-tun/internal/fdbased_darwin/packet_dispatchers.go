...
func (d *recvMMsgDispatcher) dispatch() (bool, tcpip.Error) {
	...
	// nMsgs == -1 means stopped; nMsgs == 0 means the descriptor yielded
	// nothing, which on a torn-down utun repeats forever. Returning true
	// for that case spins dispatchLoop at full CPU with no traffic and no
	// log. readVDispatcher above already guards this with `n <= 0`; match it.
	if nMsgs <= 0 {
		return false, nil
	}
