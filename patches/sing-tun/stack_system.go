...
func (s *System) tunLoop() {
	...
	for {
		...
		if err != nil {
			if E.IsClosed(err) || errors.Is(err, syscall.EBADF) {
				if errors.Is(err, syscall.EBADF) {
					s.logger.Error(E.Cause(err, "tun descriptor is gone, stopping read loop"))
				}
				return
			}
		...
		if n < header.IPv4MinimumSize {
			shortReads++
			if shortReads > maxConsecutiveEmptyReads {
				s.logger.Error(E.New("read yielded no usable packet ", maxConsecutiveEmptyReads,
					" times in a row, stopping read loop (tun descriptor is gone)"))
				return
			}
			continue
		}
	...
	for {
		...
		if len(buffers) == 0 {
			...
			if emptyReads > maxConsecutiveEmptyReads {
				s.logger.Error(E.New("batch read yielded nothing ", maxConsecutiveEmptyReads,
					" times in a row, stopping read loop (tun descriptor is gone)"))
				return
			}
			continue
