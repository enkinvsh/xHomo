	...
	for {
		...
		if err != nil {
			if E.IsClosed(err) || errors.Is(err, syscall.EBADF) {
				if errors.Is(err, syscall.EBADF) {
					m.logger.Error(E.Cause(err, "tun descriptor is gone, stopping read loop"))
				}
				return
			}
		...
		if n < header.IPv4MinimumSize {
			...
			if shortReads > maxConsecutiveEmptyReads {
				m.logger.Error(E.New("read yielded no usable packet ", maxConsecutiveEmptyReads,
	...
	for {
		...
		if len(buffers) == 0 {
			...
			if emptyReads > maxConsecutiveEmptyReads {
				m.logger.Error(E.New("batch read yielded nothing ", maxConsecutiveEmptyReads,
					" times in a row, stopping read loop (tun descriptor is gone)"))
				return
			}
			continue
